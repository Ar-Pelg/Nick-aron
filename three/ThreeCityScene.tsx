...
export const ThreeCityScene: React.FC<ThreeCitySceneProps> = ({ scrollProgress }) => {
  ...
  useEffect(() => {
    ...
    // Reduced fog density slightly for cleaner look without shadows
    ...
    // Strict cap on pixel ratio for performance
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
    
    // OPTIMIZATION: Shadows DISABLED
    ...
    // 4. Lights
    const ambientLight = new THREE.HemisphereLight(0xffffff, 0x080820, 0.8); 
    scene.add(ambientLight);

    const dirLight = new THREE.DirectionalLight(0xaaccff, 0.6); 
    dirLight.position.set(-30, 60, -30);
    // Shadows disabled on light
    scene.add(dirLight);

    const streetGlow = new THREE.DirectionalLight(0xffaa00, 0.4);
    streetGlow.position.set(20, 10, 10);
    scene.add(streetGlow);

    // 5. Materials - Switched to Lambert for performance (no PBR calculations)
    // Custom shader for water remains MeshStandard or Basic, we keep Standard for the shader hook but simplify settings
    ...
    // Use Lambert materials instead of Standard
    const materials = {
      klinkers: new THREE.MeshLambertMaterial({ color: 0x5D4037 }), 
      hardsteen: new THREE.MeshLambertMaterial({ color: 0x3E2723 }), 
      water: waterMaterial, 
      woodDark: new THREE.MeshLambertMaterial({ color: 0x261612 }), 
      woodWarm: new THREE.MeshLambertMaterial({ color: 0x4E342E }), 
      boatHull: new THREE.MeshLambertMaterial({ color: 0x3E2723 }), 
      boatWood: new THREE.MeshLambertMaterial({ color: 0x8D6E63 }),
      boatInside: new THREE.MeshLambertMaterial({ color: 0x5D4037 }),
      woodCream: new THREE.MeshLambertMaterial({ color: 0xfff3e0 }), 
      gableTrim: new THREE.MeshLambertMaterial({ color: 0xffecb3 }), 
      glass: new THREE.MeshLambertMaterial({ color: 0x111111 }),
      glassLit: new THREE.MeshLambertMaterial({ color: 0xffb74d, emissive: 0xff8f00, emissiveIntensity: 1.0 }), 
      glassRoof: new THREE.MeshLambertMaterial({ color: 0x81D4FA, transparent: true, opacity: 0.3, side: THREE.DoubleSide }),
      iron: new THREE.MeshLambertMaterial({ color: 0x1a1a1a }),
      chrome: new THREE.MeshLambertMaterial({ color: 0xaaaaaa }),
      pole: new THREE.MeshLambertMaterial({ color: 0x1f120d }), 
      amsterdammertje: new THREE.MeshLambertMaterial({ color: 0x4a1212 }), 
      leaves: new THREE.MeshLambertMaterial({ color: 0x1b3310 }), 
      glow: new THREE.MeshBasicMaterial({ color: 0xffd54f }), 
      stringLight: new THREE.MeshBasicMaterial({ color: 0xffca28 }), 
      brickBase: new THREE.MeshLambertMaterial({ color: 0xffffff }), 
      cushion: new THREE.MeshLambertMaterial({ color: 0x1565C0 }),
      saddle: new THREE.MeshLambertMaterial({ color: 0x3E2723 }),
      ...
      touristOrange: new THREE.MeshLambertMaterial({ color: 0xFF6D00 }),
      bikeFrame: new THREE.MeshLambertMaterial({ color: 0x050505 }) 
    };
    ...
    const canalGeo = new THREE.PlaneGeometry(12, canalLength, 8, 32); // Minimized segments
    const canal = new THREE.Mesh(canalGeo, materials.water);
    canal.rotation.x = -Math.PI / 2;
    scene.add(canal);
    
    // Static Geometry (Roads)
    ...
    const spoutGeo = new THREE.CylinderGeometry(0.1, 0.8, 1, 3); // Triangle cylinder
    ...
    const lightSphereGeo = new THREE.BoxGeometry(0.1, 0.1, 0.1); // Box instead of Sphere for lights
    ...
    const treeLeavesMesh = new THREE.InstancedMesh(new THREE.DodecahedronGeometry(1.5, 0), materials.leaves, maxTrees);
    scene.add(treeLeavesMesh);
    
    const poleGeo = new THREE.BoxGeometry(0.2, 0.8, 0.2); // Box instead of cylinder
    const poleMesh = new THREE.InstancedMesh(poleGeo, materials.amsterdammertje, maxPoles);
    scene.add(poleMesh);

    const lanternPostGeo = new THREE.BoxGeometry(0.15, 3.5, 0.15); 
    ...
    // Only update matrices once
    houseMesh.instanceMatrix.needsUpdate = true; 
    if (houseMesh.instanceColor) houseMesh.instanceColor.needsUpdate = true;
    gableBlockMesh.instanceMatrix.needsUpdate = true; 
    if (gableBlockMesh.instanceColor) gableBlockMesh.instanceColor.needsUpdate = true;
    trimMesh.instanceMatrix.needsUpdate = true;
    spoutMesh.instanceMatrix.needsUpdate = true; 
    if (spoutMesh.instanceColor) spoutMesh.instanceColor.needsUpdate = true;
    beamMesh.instanceMatrix.needsUpdate = true;
    frameMesh.instanceMatrix.needsUpdate = true; 
    glassMesh.instanceMatrix.needsUpdate = true; 
    litGlassMesh.instanceMatrix.needsUpdate = true;
    treeTrunkMesh.instanceMatrix.needsUpdate = true; 
    treeLeavesMesh.instanceMatrix.needsUpdate = true;
    poleMesh.instanceMatrix.needsUpdate = true; 
    dockedHullMesh.instanceMatrix.needsUpdate = true; 
    dockedCoverMesh.instanceMatrix.needsUpdate = true;
    stringLightMesh.instanceMatrix.needsUpdate = true;
    lanternMesh.instanceMatrix.needsUpdate = true; 
    lanternHeadMesh.instanceMatrix.needsUpdate = true;
    bikeMesh.instanceMatrix.needsUpdate = true; 
    bikeFrameMesh.instanceMatrix.needsUpdate = true;
    bikeHandleMesh.instanceMatrix.needsUpdate = true; 
    bikeSaddleMesh.instanceMatrix.needsUpdate = true; 
    bikeCarrierMesh.instanceMatrix.needsUpdate = true;

    const createSloop = () => {
       const g = new THREE.Group();

       // Extremely low poly hull
       const hullGeo = new THREE.CylinderGeometry(1.4, 1.2, 9.5, 8, 1, false); 
       ...
       for(let i=0; i<3; i++) { // Fewer posts
           const zPos = -2.0 + i*2.0;
       ...
       for(let r=0; r<4; r++) { // Fewer seats
           const zPos = -2.0 + r*1.2;
    ...
    const animate = () => {
      ...
      // PERFORMANCE CHECK: STOP RENDERING IF OFF SCREEN
      if (!isVisible) return;
